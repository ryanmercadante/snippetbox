...
func TestPing(t *testing.T) {
	// Create a new instance of our application struct. For now, this
	// just contains a couple of mock loggers (which discard anything
	// written to them).
	app := &application{
	...
	// server, passing in the value returned by our app.routes() method
	// as the handler for the server. This starts up a HTTPS server which
	// listens on a randomly-chosen port of your local machine for the
	// duration of the test. Notice that we defer a call to ts.Close()
	// to shutdown the server when the test finishes.
